...
((
 nb_journées_par_zone_année
 %>% mutate(nofinesset =
             ifelse(`DéptNom` == "ESSONNE",           "910019934",
             ifelse(`DéptNom` == "HAUTS DE SEINE",    "920012127",
             ifelse(`DéptNom` == "PARIS",             "750010266",
             ifelse(`DéptNom` == "SEINE ET MARNE",    "770809168",
             ifelse(`DéptNom` == "SEINE SAINT DENIS", "930000211",
             ifelse(`DéptNom` == "VAL D'OISE",        "950000364",
             ifelse(`DéptNom` == "VAL DE MARNE",      "940002231",
             ifelse(`DéptNom` == "YVELINES",          "780140026",
                    "590037768")))))))))
 %>% inner_join(GEOLOC)
) -> points_on_map)
...
(
 GEOLOC
 %>% filter(str_detect(rslongue, str_to_upper("PHARM")))
 %>% filter(str_detect(rslongue, "LOUIS"))
 %>% filter(str_starts(nofinesset, "75"))
)
...
(
 eval(str2expression(geom_sf_calls))
  ...
  + coord_sf(xlim = c(590000, 735000), ylim = c(6790000, 6900000))
)
